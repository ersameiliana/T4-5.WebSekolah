...
<?php 
    // MENGAMBIL ROLE ADMIN DARI SESSION
    $role = session()->get('role_admin') ?? 'admin_database'; 
    $nama_admin = session()->get('nama_admin') ?? 'Administrator';
    
    // Inisial untuk Avatar Profil (Ambil huruf pertama)
    $inisial = strtoupper(substr($nama_admin, 0, 1));
?>

<?= $this->section('sidebar_menu') ?>
    <style>
        /* 🔥 CSS ANTI-KAKU UNTUK SIDEBAR 🔥 */
        .nav-item { 
    ...
    <?php if (in_array($role, ['admin_editing', 'admin_database'])): ?>
    ...
    <?php if (in_array($role, ['admin_administrasi', 'admin_database'])): ?>
        ...
        <a href="<?= base_url('admin/pengguna') ?>" class="nav-item">👥 Kelola Akun</a>
        <a href="<?= base_url('admin/jurusan') ?>" class="nav-item">🏢 Kelola Jurusan</a>
    <?php endif; ?>

    <?php if ($role === 'admin_database'): ?>
...
<style>
    /* 🔥 KUSTOMISASI TOP BAR & PROFIL (POJOK KANAN) 🔥 */
    .top-bar-area {
        display: flex;
        justify-content: flex-end; /* Geser semua ke kanan */
        align-items: center;
        gap: 20px;
        margin-bottom: 30px;
    }
...
<?php 
    if ($role == 'admin_editing') {
        $nama_jabatan = "Admin Content & Editing"; $warna_tema = "#3b82f6"; $bg_badge = "rgba(59, 130, 246, 0.1)";
    } elseif ($role == 'admin_administrasi') {
        $nama_jabatan = "Admin Administrasi Data"; $warna_tema = "#f59e0b"; $bg_badge = "rgba(245, 158, 11, 0.1)";
    } else {
        $nama_jabatan = "Database Superadmin"; $warna_tema = "#9f1239"; $bg_badge = "rgba(159, 18, 57, 0.1)";
    }
?>
...
<div class="top-bar-area">
    <div class="profile-info">
        <div>
            ...
            <div style="font-size: 0.75rem; color: #94a3b8; font-weight: 600; text-transform: uppercase;"><?= str_replace('_', ' ', $role) ?></div>
        </div>
...
<div class="glass-card">
    <span class="role-badge" style="background: <?= $bg_badge ?>; color: <?= $warna_tema ?>; border: 1px solid <?= $warna_tema ?>;">
        🛡️ LEVEL: <?= strtoupper(str_replace('_', ' ', $role)) ?>
    </span>
    ...
    <?php if ($role === 'admin_editing'): ?>
        <div style="background: rgba(59, 130, 246, 0.1); border-left: 4px solid #3b82f6; padding: 15px; margin-top: 20px; border-radius: 0 10px 10px 0;">
            <p style="color: #60a5fa; font-size: 0.95rem; margin: 0;">📝 <strong>Hak Akses Anda:</strong> Anda memiliki wewenang untuk menambah, mengubah, dan menghapus konten Publikasi. Pastikan konten bebas dari *typo* sebelum di-<em>publish</em>.</p>
        </div>
    <?php elseif ($role === 'admin_administrasi'): ?>
        <div style="background: rgba(245, 158, 11, 0.1); border-left: 4px solid #f59e0b; padding: 15px; margin-top: 20px; border-radius: 0 10px 10px 0;">
            <p style="color: #fbbf24; font-size: 0.95rem; margin: 0;">🔒 <strong>Peringatan Akses:</strong> Anda berwenang mengelola data Pengguna. <strong>Namun, wewenang Hapus (Delete) dikunci.</strong> Silakan hubungi Admin Database jika ada data yang wajib dihapus.</p>
        </div>
    <?php else: ?>
        <div style="background: rgba(159, 18, 57, 0.1); border-left: 4px solid #9F1239; padding: 15px; margin-top: 20px; border-radius: 0 10px 10px 0;">
            <p style="color: #ff4d4d; font-size: 0.95rem; margin: 0;">⚠️ <strong>Perhatian Superadmin:</strong> Anda memiliki akses tertinggi (Full CRUD). Perubahan atau penghapusan data pada panel ini akan langsung memicu *Trigger* MySQL dan memengaruhi *database* utama.</p>
        </div>
    <?php endif; ?>
</div>
